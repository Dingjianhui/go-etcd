...
func main()  {
	...
	// 3.将服务注册到etcd
	go func() {
		err := service.RegService(serviceId,serviceName,serviceAddress + ":" + strconv.Itoa(servicePort))
		if err != nil {
			errChan <- err
		}
	}()
	...
	go func() {
		err := service.UnRegService(serviceId) // 服务反注册 (将服务从etcd中删除,释放资源)
		if err != nil {
